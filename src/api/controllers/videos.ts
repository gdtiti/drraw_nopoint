...
export async function generateVideo(
  ...
  // 检查每日使用限制
  const usageCheck = await sessionUsageDB.checkUsageLimit(effectiveSessionId, 'video');
  if (!usageCheck.allowed) {
    throw new APIException(EX.API_VIDEO_GENERATION_FAILED,
      `今日视频生成次数已达上限 (${usageCheck.current}/2)。请明天再试。`);
  }
  ...
  // 如果生成成功，增加使用计数
  try {
    await sessionUsageDB.incrementUsage(effectiveSessionId, 'video');
    logger.info(`Session ${effectiveSessionId} 视频生成成功，剩余次数: ${usageCheck.remaining - 1}`);
  } catch (error) {
